...
num = 0;
  sprintf(filebuf, "%s/%s", BBSHOME, PASSFILE);
	 ...
	 // sleep(1);
	  ...
	  fclose (fp);
		sprintf(buf,"%s %d %d %d\n",fboy.userid ,fboy.inbank , fboy.money,fboy.inbank + fboy.money);
	  printf(buf);
		fboy.money = fboy.inbank + fboy.money;
		if(fboy.money < 0) fboy.money = 0;
		if(fboy.money >3000000) fboy.money = 300000;
		fboy.inbank = 0;
		tmp = (char *)&fboy;
