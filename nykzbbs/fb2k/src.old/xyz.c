...
int ent_sortdir2 (void)
{
    ...
    clear ();
    stand_title ("°åÃæÎÄÕÂÅÅĞò");
//    if(!get_a_boardname (buf, "ÊäÈëÌÖÂÛÇøÃû (°´¿Õ°×¼ü×Ô¶¯ËÑÑ°): "))
//		return 0;
    make_blist();
    namecomplete("ÊäÈëÌÖÂÛÇøÃû (°´¿Õ°×¼ü×Ô¶¯ËÑÑ°): ", buf);
    if (*buf == '\0')
    {
        return 0;
    }
	...
	{
		move(3, 0);
		prints("¶Ô²»Æğ£¬ÄãÃ»ÓĞÈ¨ÏŞ¶Ô´Ë°åÃæÅÅĞò\n");
		pressreturn();
		return 0;
	}
	sprintf(sysmsg, "¸ø %s °åÅÅĞò", buf);
...
void anywhere_lockscreen(void)
{
    ...
    while (*buf == '\0' || !checkpasswd(currentuser.passwd, buf))
    {
        ...
        move(t_lines - 1, 87);
        prints
            ("[33m[[36mËøÆÁÖĞ[33m][32m ÇëÊäÈëÃÜÂë[33m:[m               ");
