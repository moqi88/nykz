    ...
    if (FFLL == 0)
    {
        if (!strncmp("Re: ", ent->title, 4))
        {
            sprintf(buf, " %5d %s%c%s %-12.12s %s%6.6s\033[m %s%-.48s ",
                    ...
                    date, noreply ? "\033[0;1;4;33mx\033[m" : " ", TITLE);
        }
        else
        {
...
int read_pushpost(int ent, struct fileheader *fileinfo, char *direct)
{
    ...
    {
        prints
            ("\033[1;44;31m[ÔÄ¶ÁÎÄÕÂ]  \033[33m»ØÐÅ R ©¦ ½áÊø Q,¡û ©¦ÉÏÒ»·â ¡ü©¦ÏÂÒ»·â <Space>,¡ý©¦Ö÷ÌâÔÄ¶Á ^X»òp \033[m");
    }
    else
    {
        prints
            ("\033[1;44;31m[ÔÄ¶ÁÎÄÕÂ]  \033[33m½áÊø Q,¡û ©¦ÉÏÒ»·â ¡ü©¦ÏÂÒ»·â <Space>,<Enter>,¡ý©¦Ö÷ÌâÔÄ¶Á ^X »ò p \033[m");
    }
             ...
             int is_module)
{
    ...
    if (!junkboard())
    {
		...
		else
		{
			...
			post_money = 20;
			post_money = (post_money+(post_time*post_word)/post_money)/2;
			post_money = (post_money+(post_time*post_word)/post_money)/2;
			post_money = (post_money+(post_time*post_word)/post_money)/2;
			...
			if((post_time/(2*post_word)>9) || ((2*post_word)/post_time>9)){
				if(post_money > 20) post_money = 20;
			}
...
int edit_title(int ent, struct fileheader *fileinfo, char *direct)
{
    ...
    if (buf[0] != '\0')
    {
        ...
        fprintf(fp,
                ...
                currentuser.userid, currenttime, currentuser.lasthost);
        fclose(fp);
...
int flag_clearto(int ent, char *direct, int clearall)
{
 ...
 //   if (uinfo.mode != READING)
 //       return DONOTHING;//by neige for c in fav
