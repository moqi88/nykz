...
int rate_edit(void)
{
	...
	if(fd=open("bank/rate",O_RDWR | O_APPEND)!=-1)
	{
...
int send_salary(void)
{
	...
	if(askyn ("确定发放薪水？", NA, NA) == YEA)
	{
		...
		while (size > 0)
		{
			...
			sprintf(buf,"尊敬的%s, 您本次的薪水为 %d ",sal.userid, sal.money);
			//bankreport("发工资啦！", buf, lookupuser.userid, NULL);
			
			autoreport ("薪水发放", buf, lookupuser.userid, NULL);
			sprintf(buf,"%s 的薪水为 %d \n", lookupuser.userid, sal.money);
			strcat (mail_buf, buf);
			size--;
		}
		...
		sprintf(buf,"薪水已经发放(by %s)", currentuser.userid);
		securityreport(buf);
		/*
...
int festival_money(){
	...
	if (askyn ("确认要给全站发红包吗？", YEA, NA) == YEA)
	{
		...
		close(fd);
		i=1;
		sprintf(fname, "思源实业银行发给您一个红包，红包内有 %d 元", money);
		//sprintf(fname, "思源实业银行发给您一个红包，红包内有 xxxxx 元");
		...
		while(i<=size )
		{
			...
			autoreport ("思源实业银行发给您一个红包", fname, user.userid, NULL);
			i++;
		}
