        ...
        else if (strncmp(buf, "Path=", 5) == 0)
        {
            ...
            if ((!strstr(litem.title + 38, "(BM: BMS)")
                ...
                && (!strstr(litem.title + 38, "(BM: SYSOPS)")
                    || HAS_PERM(PERM_SYSOP))&&!strstr(litem.title + 38, "(BM: -"))
            {
            ...
            //added by neige
           	...
           	if (strstr(litem.title + 38, "(BM: -"))
       		{
           		checkBM=strstr(litem.title + 38, "(BM: -");
           		strncpy(bmstr,checkBM,STRLEN);
...
void a_iteminfo(MENU *pm)
{
    ...
    //add by Neige
        ...
        //add by Neige
        sprintf (tmpline, "¾«»ªÇøÎÄÕÂ±à¼­ÈÕÆÚ: [1m%02d[mÄê[1m%02d[mÔÂ[1m%02d[mÈÕ [1m%02d[m:[1m%02d[m\n",
                     pt->tm_year % 100, pt->tm_mon + 1, pt->tm_mday, pt->tm_hour, pt->tm_min);
        prints (tmpline);//
    }
    ...
    case 'S':
        {
            ...
            while (i < 4)
            {
                ...
                {
                    sprintf(genbuf, "µÚ%dË¿Â·: %s", i + 1, " ÉÐÎ´ÉèÖÃ");
                }
                prints(genbuf);
                i++;
            }
