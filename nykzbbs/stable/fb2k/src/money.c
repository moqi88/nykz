...
//	prints("\n[1]»îÆÚ´æ¿î [2]»îÆÚÈ¡¿î [3]¹ºÂòÓÎÏ·±Ò [4]³öÊÛÓÎÏ·±Ò [5]¶¨ÆÚ´æ¿î [6]¶¨ÆÚÈ¡¿î\n");
	...
	if(opt == 2)
	{
		...
		sprintf (mail_buffer,"ÏÖÔÚ¿ÉÌáÈ¡µÄÊµ¼Ê½ğ¶î£º%d ºş±Ò\n",currentuser.inbank);
		prints (mail_buffer);
		...
		/*¸üĞÂPASSFILEÎÄ¼ş*/
		currentuser.money += num_money;//Ôö¼Ó±¾½ğ
...
*/	
	...
	if(opt == 5)//	if(opt == 6)
	{
		...
		if(Bill.dateout < Bill.datein + Bill.length*86400*30)
		{
			...
			{
				num_money=(int)Bill.money*(1+callrate*(double)(Bill.dateout-Bill.datein)/86400);//°´ÕÕ»îÆÚµÄËã
			}
		}
		else
		{
			...
			num_money =(int)Bill.money*(1+BANK_CURRDEPO_RATE[Bill.length-1]);//°´ÕÕ¶¨ÆÚµÄËã
			num_money +=(int)Bill.money*(callrate*(double)(Bill.dateout-(Bill.datein + Bill.length*86400*30))/86400);//°´ÕÕ»îÆÚµÄËã			
...
int shopping()//modified by pax, 2005.09.21.
{
	...
	move(counts/2+6,0);
	prints("ÄúÄ¿Ç°ÉĞÓĞ %d Ôª (½±ÕÂ %d ¸ö)\n",	currentuser.money,currentuser.medals);
	if (!gettheuserid(counts/2+7,"ÄúÏë¸øË­ÂòÀñÎï£¿ÇëÊäÈëËû/ËıµÄÕÊºÅ: ",&id))
	{
		prints("\nÊäÈë´íÎó¡£È¡Ïû½»Ò× ...");
		pressanykey();
		outgame();
	...
	getdata(counts/2+8, 0, "ÄúÏë¹ºÂòÄÄÒ»¼şÉÌÆ·£¿", ans, 3, DOECHO, YEA);
	no = atoi(ans);
	if ( no < 1 || no > counts ) {
		prints("\nÊäÈë´íÎó¡£È¡Ïû½»Ò× ...");
		pressanykey();
