...
/* ---------------------------------------------- */
	...
	//add by pax
    sprintf(curfile, "etc/posts/day_link");
    if ((fp2 = fopen(curfile, "w")) != NULL) {}
...
//            j = i;
        ...
        for (i = 0; i < j && real < mytop[mytype]; i++) {
            ...
            if (mytype == 0) {  /*只有当日十大才做下面的检查 */
                ...
                if (exist)
				{
					strcpy(top[i].realPath,fh->filename);
                    continue;
				}
            ...
            real++;
				...
				//add by pax
				fprintf(fp,"%3d%-16s%s%4d%16%-60.60s%20s\n", !mytype ? real : (i + 1), top[i].board, p, top[i].number, top[i].author, top[i].title,top[i].realPath);
