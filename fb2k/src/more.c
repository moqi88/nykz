...
struct more_link more_links[1024];
...
int parse_more_link(char *buf, int *bytes, int y, int start_x, int buildlinks, char *seek_start)
{
    ...
    if (buildlinks)
    {
        ...
        strcpy(more_links[more_links_count].dest, dest);
...
display_more_screen(int fd, int topline)
{
    ...
    numbytes = readln(fd, readln_buf, 0, 0);
    relative_y = 1;
    currline = topline + relative_y - 1;
    if (currline < bottomline)
        secondnum = readln(fd, second_buf, 0, 0);
    else
    {
        second_buf[0] = 0;
        secondnum = 0;
    ...
    if (maxline >= t_lines)
    {
        sprintf(buf2, "(%d%%) %d-%dÐÐ", (viewedbytes * 100) / maxbytes,
                topline, bottomline);
        prints("[1;44;32m%s%-18s[33m| ½áÊø¡û ÒÆ¶¯¡ü¡ýPgUp/Dn Home,End Ìø×ª[ ] ÇóÖúh |  [m",
               ((bottomline < maxline) && !(viewedbytes / maxbytes)) ? "ÏÂÃæ»¹ÓÐà¸" : "ÏÂÃæÃ»ÓÐÁË", buf2);
    }
    else if (!promptend)
    {
        if (more_mode == MORE_MODE_SREAD)
            prints("[1;44;31m[Ö÷ÌâÔÄ¶Á] [33mÏÂÒ»·â <Space>,<Enter>,¡ý©¦ÉÏÒ»·â ¡ü,U                              [m");
        else if (more_mode == MORE_MODE_AUSER)
            prints("[1;44;31m[ÏàÍ¬×÷Õß] [33mÏÂÒ»·â <Space>,<Enter>,¡ý©¦ÉÏÒ»·â ¡ü,U                              [m");
