...
int do_copyfav(char *boardfile,char *fromdir, char * todir)
{
	...
	for(i = 0; i < currentdir.subnum; i++)
	{
		...
		system(buf);
		do_copyfav(currentdir.subdirsfile[i],fromdir,todir);		
	}
    if (currentdir.extends[0] != 0)
    {
		snprintf(buf,STRLEN*2,"cp %s%s %s%s",fromdir,currentdir.extends,todir,currentdir.extends);
		system(buf);
		do_copyfav(currentdir.extends,fromdir,todir);
    }
	return 0;
}
...
int CopyMainHck()
{
    ...
    snprintf(destfilename,STRLEN, "home/%c/%c/%s/",
             toupper(touser[0]),
             toupper(touser[1]),touser);
    snprintf(buf,STRLEN*2,"cp %smain.hck %smain.hck.%s",srcfilename,destfilename,currentuser.userid);
    system(buf);
    snprintf(buf,STRLEN*2,"cp %s.dirnum %s.dirnum.%s",srcfilename,destfilename,currentuser.userid);
    system(buf);
    return 0;    
}
int PasteMainHck()
{
    ...
    system(buf);
    snprintf(buf,STRLEN*2,"cp %s.dirnum.%s %s.dirnum",destfilename,fromuser,destfilename);
    system(buf);
